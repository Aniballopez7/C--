...
void buscarJuego(NodoJuegos *&listaJuegos, int idJuegos)
{
    ...
    while (clonLista != NULL)
    {
        if (clonLista->idJuego == idJuegos)
        {
            ...
            if (encontrado = true)
            {
                cout<<"\nEl juego de "<<clonLista->nombreJuego<<" se ha econtrado en el catalogo de juegos"<<endl;
                break;
            }
        }
        clonLista = clonLista->siguiente;
...
void listaJuegosClientes(juegosCliente *&listajuegosClientes, NodoJuegos *&listaJuegos, string nombre, string usuarioC)
{
    ...
    cout<<"\nSe Ha agregado a su lista de juegos\n"<<endl;
...

{
    ...
    else
    {
        ...
        if (encontrado == false)
        {
            cout<<"\nUsted "<<usuarioC<<" no ha adquirido ningun juego\n"<<endl;
        }
    }
}
        ...
        case 1:
        {
            ...
            if (cont == 0)
            {
                ...
                while(!(cin>>precio))
                {
                	cin.clear();
                	cin.ignore(1000,'\n');
                    cout<<"\nIngrese un dato de tipo numerico\n"<<endl;
